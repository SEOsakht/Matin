...
export default function AnchorAccordion({ anchorId }: { anchorId: string }) {
  ...
  async function fetchAnnotations(){ const { data, error } = await supabase.from('annotations').select('*').eq('anchor_id', anchorId).order('created_at',{ascending:false}).limit(200); if(error) console.error(error); else setAnnotations(data||[]) }
  async function postAnnotation(parentId: string|null=null){ const { data: { user } } = await supabase.auth.getUser(); if(!user){ alert('لطفا وارد شوید'); return } ; const nick = await supabase.from('matin_users').select('id,nickname').eq('supabase_id', user.id).maybeSingle(); let authorId=null, authorName='ناشناس'; if(nick.data){authorId=nick.data.id; authorName=nick.data.nickname} const { data, error } = await supabase.from('annotations').insert([{ anchor_id: anchorId, parent_id: parentId, author_id: authorId, author_name: authorName, body: newBody }]).select().single(); if(error){console.error(error); alert('خطا')}else{ setNewBody(''); setAnnotations(prev=>[data,...prev]) }
  }
  return (
